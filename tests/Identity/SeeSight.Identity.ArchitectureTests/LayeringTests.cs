...
public sealed class LayeringTests
{
    ...
    private static readonly System.Reflection.Assembly ApiAssembly = typeof(AuthController).Assembly;
    ...
    {
        // The Domain layer is deliberately kept pure so it's unit-testable with
        // zero infrastructure (docs/Microservices.md §8) — verified here, not
    ...
    public void Application_has_no_provider_specific_infrastructure_dependency(string forbiddenNamespace)
    {
        ...
        // IIdentityDbContext — docs/ProjectReferenceDiagram.md §6) but never a
        // concrete provider or infrastructure library — those belong in
        // Infrastructure, injected back via Dependency Inversion.
        var result = Types.InAssembly(ApplicationAssembly)
            .Should().NotHaveDependencyOn(forbiddenNamespace)
            .GetResult();
    ...
    public void No_layer_depends_on_the_gateway()
    {
        ...
        // another service's (or the Gateway's) project.
